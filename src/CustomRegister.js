...
// Your user pool ID (from your Cognito configuration)
const userPoolId = "us-east-1_dAD3lRUXx";
...
function CustomRegister() {
  ...
  const [view, setView] = useState("register"); // "register" or "confirm"
  const [topics, setTopics] = useState([]); // For testing API call after confirmation
  const [users, setUsers] = useState([]); // For listing users from Cognito
  ...
  const fetchTopics = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/getTopics`);
  ...
  const fetchUsers = async () => {
    try {
      const input = { UserPoolId: userPoolId };
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (response["$metadata"]?.httpStatusCode === 200) {
        setSuccessMsg("Email confirmed successfully! You can now log in.");
        // Fetch topics and users after confirmation
        await fetchTopics();
        await fetchUsers();
        // Redirect to homepage after 10 seconds (adjust delay as needed)
        setTimeout(() => {
          navigate("/");
        }, 10000);
      }
  ...
  return view === "register" ? (
    ...
    <div className="card">
      ...
      {/* Display users fetched from Cognito */}
      {users.length > 0 && (
        <div>
          <h2>Users:</h2>
          <ul>
            {users.map(user => {
              // Extract the sub and email from the user's attributes
              const subAttr = user.Attributes.find(attr => attr.Name === "sub");
              const emailAttr = user.Attributes.find(attr => attr.Name === "email");
              return (
                <li key={user.Username}>
                  <strong>Username:</strong> {user.Username} | 
                  <strong> Sub:</strong> {subAttr ? subAttr.Value : "N/A"} | 
                  <strong> Email:</strong> {emailAttr ? emailAttr.Value : "N/A"}
                </li>
              );
            })}
          </ul>
